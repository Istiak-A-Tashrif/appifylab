  ...
  },
  {
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="27" height="20" fill="none" viewBox="0 0 27 20">
        <path className="_dark_svg" fill="#000" fillOpacity=".6" fillRule="evenodd" clipRule="evenodd" d="M13.334 12.405h.138l.31.001c2.364.015 7.768.247 7.768 3.81 0 3.538-5.215 3.769-7.732 3.784h-.932c-2.364-.015-7.77-.247-7.77-3.805 0-3.543 5.405-3.774 7.77-3.789l.31-.001h.138zM13.334 0c3.086 0 5.596 2.442 5.596 5.442 0 3.001-2.51 5.443-5.596 5.443H13.3a5.616 5.616 0 01-3.943-1.603A5.308 5.308 0 017.74 5.439C7.739 2.442 10.249 0 13.334 0z" />
      </svg>
    ),
  },
  {
          ...
          <div className="row">
            <div className="col-xl-12 col-lg-12 col-md-12">
              <ul className="_mobile_navigation_bottom_list">
                {NAV_ITEMS.map((item) => (
                  <li className="_mobile_navigation_bottom_item" key={item.id}>
                    <a
                      href={item.href}
                      className={
                        "_mobile_navigation_bottom_link" +
                        (active === item.id ? " _mobile_navigation_bottom_link_active" : "")
                      }
                      onClick={(e) => {
                        e.preventDefault();
                        setActive(item.id);
                    ...
                    >
                      {item.icon}
                      {item.counting != null && <span className="_counting">{item.counting}</span>}
                    </a>
                  </li>
                ))}
